...
describe('opencode-live commands (direct handlers)', () => {
  ...
  it('rejects secret-looking command arguments without storing or echoing them', async () => {
    const catalog = new CatalogManager({ config: testCatalogConfig() })
    const stored: string[] = []
    const definitions = commandDefinitions({} as Context, {
      catalog,
    ...
    expect(withKey?.kind === 'error' ? withKey.text : '').not.toContain('sk-test-123')
    catalog.stop()
  })

  it('rejects arbitrary command arguments without attempting validation or storage', async () => {
    const catalog = new CatalogManager({ config: testCatalogConfig() })
    const stored: string[] = []
    const definitions = commandDefinitions({} as Context, {
      catalog,
    ...
    const enable = definitions.find(definition => definition.name === 'dsh-opencode')
    const blank = await enable?.handler({ rawInput: 'unexpected-argument', signal: new AbortController().signal } as never)
